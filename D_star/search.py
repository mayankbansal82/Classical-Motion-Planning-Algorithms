...
class DStar:
    ...
    def process_state(self):
        ...
        if node.k < node.h:
            for y in neighbors:
                if y.tag == "NEW" or (y.parent == node and y.h != (node.h + self.cost(node,y))):
                    # y.tag = "OPEN" 
                    # y.k = node.h + self.cost(node,y)
                    self.insert(y,node.h + self.cost(node,y))
                elif y.parent != node and y.h > node.h + self.cost(node,y):
                    # node.tag = "OPEN"
                    node.k = node.h
                    self.insert(node,node.h)
                elif y.parent != node and node.h > y.h + self.cost(node,y):
                    # y.tag = "OPEN"
                    y.k = y.h
                    self.insert(y,y.h)
        ...
        # If node k is the same as h (LOWER)
            ...
            for y in neighbors:
                if y.tag == "NEW" or y.h > node.h + self.cost(node,y):
                    # y.h = node.h + self.cost(node,y)
                    y.parent = node
                    # y.tag = "OPEN"
                    # y.k = y.h
                    self.insert(y,node.h + self.cost(node,y))
    ...
    def repair_replan(self, node):
        ...
        while(k_min >= node.h) or len(self.open)!=0:
    ...
    def modify_cost(self, obstacle_node, neighbor):
        ...
        for node in neighbor:
            self.insert(node,obstacle_node.h + self.cost(node,obstacle_node))
    ...
    def prepare_repair(self, node):
        ...
        for neighbor in self.get_neighbors(node):
            if neighbor.is_dy_obs == True and neighbor.is_obs == False:
                self.modify_cost(neighbor,self.get_neighbors(neighbor))
    ...
    def run(self):
        ...
        while(not ((k_min == -1) or (cur_state.tag == "CLOSED"))):
            k_min = self.process_state()
        # self.path = self.get_backpointer_list(self.start)
        ...
        while(cur_state!=self.goal):
            self.prepare_repair(cur_state)
            self.repair_replan(cur_state)
            self.get_backpointer_list(cur_state)
            # Visualize the path in progress
            # print(self.path)
